/**
 * @file zxpac4d.cpp
 * @brief zxpac4d for ASCII only data and binary data
 * @version 0.1
...
zxpac4d::zxpac4d(const lz_config* p_cfg, int ins, int max) :
    lz_base(p_cfg),
...
int zxpac4d::lz_search_matches(char* buf, int len, int interval)
{
    int pos = 0;
    int num;
    int offset, length;
...
int zxpac4d::lz_parse(const char* buf, int len, int interval)
{
    int length;
    int offset;
    int pos;
...
void zxpac4d::preload_tans(int type, uint8_t* freqs, int len)
{
}


const cost* zxpac4d::lz_cost_array_get(int len)
{
    if (len < 1) {
       return NULL;
    }
...
void zxpac4d::lz_cost_array_done(void)
{
    if (m_alloc_len > 0) {
...
int zxpac4d::encode_history(const char* buf, char* p_out, int len, int pos)
{
    char literal;
    int tag;
    int length;
    ...
    while ((pos = m_cost_array[pos].next)) {
        ...
        if (offset == 0 && length == 1) {
            ...
            if (m_lz_config->debug_level > DEBUG_LEVEL_NORMAL) {
                std::cerr << ", bits(0,1), 0x" << std::hex << std::setfill('0');
				std::cerr << std::right << std::setw(2) << (literal & 0xff);
...
int zxpac4d::lz_encode(char* buf, int len, char* p_out, std::ofstream* ofs)
{
    int n;
	(void)ofs;
