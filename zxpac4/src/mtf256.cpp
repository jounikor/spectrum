 ...
 *
 * @note This is a brute force implementation meaning there's just one
 *       flat array for nodes and all operations takes place in it. This
 *       implies unnecessary copying and scanning in the array.
 *
...
int mtf256::state_load(void *ctx, int len)
{
    if (len != (m_encodeable - m_reserved)) {
        return -1;
    }

    std::memcpy(m_arr,ctx,len);
    return 0;
}



int mtf256::state_save(void *ctx, int len)
{
    (void)ctx;
    (void)len;
    return -1;
