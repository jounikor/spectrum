...
zxpac4_cost::~zxpac4_cost(void)
{
}

int zxpac4_cost::impl_get_offset_tag(int offset, char& byte_tag, int& bit_tag)
{
    if (offset < 128) {
        // (0ooooooo)
        byte_tag = offset;
...
int zxpac4_cost::impl_get_offset_bits(int offset)
{
...
int zxpac4_cost::impl_get_literal_bits(char literal, bool is_ascii)
{
    (void)literal;
...
int zxpac4_cost::impl_literal_cost(int pos, cost* c, const char* buf)
{
    cost_with_ctx* p_ctx = &static_cast<cost_with_ctx*>(c)[pos];
...
int zxpac4_cost::impl_match_cost(int pos, cost* c, const char* buf)
{
    ...
    for (int n = 0; n < p_ctx->num_matches; n++) {
        ...
        if (offset == pmr_offset) {
            // encoding as a PMR and in that case minimum length is 1
            offset = 0;
            encode_length = length;
            assert(encode_length > 0);
        } else {
            // encoding as a match and in that case minimum length is 2
            pmr_offset = offset;
            encode_length = length - 1;
            assert(encode_length > 0);
        }

        // Check for PMR and offset cost.. 
        if (offset >= 131072) {
            std::cerr << "**Error: " << pos << ", " << offset << ":" << length << std::endl;
            assert(offset < 131072);
        }
...
int zxpac4_cost::impl_init_cost(cost* c, int sta, int len, int pmr)
{
